...
class WarpGate extends Level {
  build(meshes, transparentMeshes) {
    ...
    for (let i = 0; i < 2; i += 1 / 8) {
        ...
        this.drawRoadLine(this.roadPath, linePath1, x * 4, 0.15, RoadLineStyle.DASH(10, 10 + i * 10), 0, 1, 0);
        const mesh1 = makeMesh(linePath1, 0.1, 0, 0.5);
        mesh1.position.z = y * 4;
        meshes.push(mesh1);

        const linePath2 = new THREE.ShapePath();
        this.drawRoadLine(this.roadPath, linePath2, x * 20, 0.15, RoadLineStyle.DASH(5, 50 + i * 50), 0, 1, 0);
